...
class Paths {
    /// 下载路径
    static let resourceDir = rootPath.appending(path: "Resources")
    /// Wine的可执行文件路径
    static let wineDir = rootPath.appending(path: "Wine")
    /// WINEPREFIX 路径
    static let prefixDir = rootPath.appending(path: "Prefix")
    static let wineBakDir = rootPath.appending(path: "BackupDlls")
    
    /// 通过前方的绝对路径和后方的相对路径来拼凑路径
    static func path(front: String, relative: String) -> URL {
        return URL(fileURLWithPath: front).appending(path: relative)
    }
    ...
    static func checkBinaryFiles() {
        do {
            ...
            for file in execFiles {
                if !FileHelper.checkExists(file: resourceDir.appending(path: file)) {
