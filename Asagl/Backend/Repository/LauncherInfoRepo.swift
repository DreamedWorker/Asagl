...
class LauncherInfoRepo: DailyStorage {
    ...
    func fetchFromNetwork() async throws -> LauncherBasic {
        ...
        FileHelper.writeData(data: data, url: Paths.path(front: Paths.resourceDir.toPath(), relative: FILE_NAME))
    ...
    func fetchFromDisk() -> LauncherBasic? {
        let formattedOne = try? JSONDecoder()
            .decode(
                LauncherBasic.self,
                from: Data(contentsOf: Paths.path(front: Paths.resourceDir.toPath(), relative: FILE_NAME))
            )
        return formattedOne
    }
}
