...
class LauncherInfoRepo: DailyStorage {
    private let WEBSTATIC_LAUNCHER_BGS =
        URL(string: "https://hyp-api.mihoyo.com/hyp/hyp-connect/api/getAllGameBasicInfo?launcher_id=jGHBHlcOq1")!
    ...
    func fetchFromNetwork() async throws -> LauncherBasic {
        let (data, _) = try await URLSession.shared.data(from: WEBSTATIC_LAUNCHER_BGS)
